...
int main(int argc, char* argv[]) {
  ...
  char const* applicationName = "izgProjekt2017";
  ...
  while (running) {
    ...
    while (SDL_PollEvent(&event)) {
      ...
      if (event.type == SDL_MOUSEMOTION)
        onMouseMotion(event.motion.x, event.motion.y, event.motion.xrel,
                      event.motion.yrel);

      if (event.type == SDL_MOUSEBUTTONDOWN)
        onMouseButtonDown(event.button.button);

      if (event.type == SDL_MOUSEBUTTONUP) onMouseButtonUp(event.button.button);
    }
